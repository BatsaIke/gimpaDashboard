...
import {
  ...
  getAllowedParents,
  getRoleDeptOptions,
  canEdit as utilCanEdit,
  canDelete as utilCanDelete,
} from "./departmentPageUtils";
...
const DepartmentPage = () => {
  ...
  // ✅ Wrap items in useMemo so it's stable across renders
  const items = useMemo(
    () => (Array.isArray(deptState.items) ? deptState.items : []),
    [deptState.items]
  );
  ...
  const scopedItems = useMemo(() => {
    return computeScopedItems(items, authUser, isTop);
  }, [items, authUser, isTop]);

  const filteredItems = useMemo(() => {
    return filterItems(scopedItems, searchTerm, filterCategory);
  }, [scopedItems, searchTerm, filterCategory]);

  const categories = useMemo(() => {
    return getCategories(scopedItems);
  }, [scopedItems]);

  const allowedParents = useMemo(
    () => getAllowedParents(items, scopedItems, isTop),
    [isTop, items, scopedItems]
  );

  const roleDeptOptions = useMemo(
    () => getRoleDeptOptions(items, scopedItems, isTop),
    [isTop, items, scopedItems]
  );

  const canEdit = (dept) => utilCanEdit(dept, authUser, isTop);

  const canDelete = () => utilCanDelete(authUser);
